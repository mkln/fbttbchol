...
C
C           WRITE(*,*) "MB02CX loop I=1,K ", I
            IF ( Q.GT.1 ) THEN
C              WRITE(*,*) "found Q.GT.1 "
               CALL DLARFG( MIN( I, Q ), B(I,1), B(I,2), LDB, TAU )
               ALPHA  = B(I,1)
C              WRITE(*,*) "got ALPHA =", ALPHA
               B(I,1) = ONE
               IF ( K.GT.I )
...
C           
            BETA = A(I,I)
C           WRITE(*,*) "ENTERING MA02FD: ", BETA, ALPHA
C
C           condition for INFO = 1 for MA02FD is
C           ABS( ALPHA ).GE.ABS( BETA )
C           
            CALL MA02FD( BETA, ALPHA, C, S, IERR )
            IF ( IERR.NE.0 ) THEN
C
